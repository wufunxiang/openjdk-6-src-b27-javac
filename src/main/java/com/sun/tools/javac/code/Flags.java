...
public class Flags {
	...
	public static String toString(long flags) {
		...
		if ((flags & IPROXY) != 0)
			buf.append("iproxy ");
		if ((flags & NOOUTERTHIS) != 0)
			buf.append("noouterthis ");
		if ((flags & EXISTS) != 0)
			buf.append("exists ");
		if ((flags & COMPOUND) != 0)
			buf.append("compound ");
		if ((flags & CLASS_SEEN) != 0)
			buf.append("class_seen ");
		if ((flags & SOURCE_SEEN) != 0)
			buf.append("source_seen ");
		if ((flags & LOCKED) != 0)
			buf.append("locked ");
		if ((flags & UNATTRIBUTED) != 0)
			buf.append("unattributed ");
		if ((flags & ANONCONSTR) != 0)
	...
	/**
	 * Flag for annotation type symbols to indicate it has been checked and
	 * found acyclic.
	 */
	...
	/**
	 * Flag that marks a hypothetical method that need not really be generated
	 * in the binary, but is present in the symbol table to simplify checking
	 * for erasure clashes.
	 */
	...
	public static final int AccessFlags = PUBLIC | PROTECTED | PRIVATE,
			...
			ConstructorFlags = AccessFlags, InterfaceMethodFlags = ABSTRACT
					| PUBLIC, MethodFlags = AccessFlags | ABSTRACT | STATIC
					| NATIVE | SYNCHRONIZED | FINAL | STRICTFP;
