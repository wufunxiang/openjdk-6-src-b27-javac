...
public class MandatoryWarningHandler {
	 ...
	 * mandatory warning is suppressed because too many warnings have already
	 * been output.
	 * 
	...
	private enum DeferredDiagnosticKind {
		/**
		 * This kind is used when a single specific file is found to have
		 * warnings and no similar warnings have already been given. It
		 * generates a message like: FILE has ISSUES
		 */
		IN_FILE(".filename"),
		/**
		 * This kind is used when a single specific file is found to have
		 * warnings and when similar warnings have already been reported for the
		 * file. It generates a message like: FILE has additional ISSUES
		 */
		ADDITIONAL_IN_FILE(".filename.additional"),
		/**
		 * This kind is used when multiple files have been found to have
		 * warnings, and none of them have had any similar warnings. It
		 * generates a message like: Some files have ISSUES
		 */
		IN_FILES(".plural"),
		/**
		 * This kind is used when multiple files have been found to have
		 * warnings, and some of them have had already had specific similar
		 * warnings. It generates a message like: Some files have additional
		 * ISSUES
		 */
	 ...
	 *            instances should be given, or whether an aggregate message
	 *            should be generated at the end of the compilation. Typically
	 *            set via -Xlint:option.
	 * @param enforceMandatory
	 *            True if mandatory warnings and notes are being enforced.
	 * @param prefix
	 *            A common prefix for the set of message keys for the messages
	...
	public void reportDeferredDiagnostic() {
		if (deferredDiagnosticKind != null) {
			...
			if (!verbose)
				logMandatoryNote(deferredDiagnosticSource, prefix
						+ ".recompile");
		}
	}
	 ...
	 * diagnostic will be. Initially as specific and helpful as possible, as
	 * more warnings are reported, the scope of the diagnostic will be
	 * broadened.
	 */
	 ...
	 * message, based on deferredDiagnosticKind. This variable should normally
	 * be set/updated whenever deferredDiagnosticKind is updated.
	 */
	...
	/**
	 * Reports a mandatory warning to the log. If mandatory warnings are not
	 * being enforced, treat this as an ordinary warning.
	 */
	 ...
	 */
	private void logMandatoryNote(JavaFileObject file, String msg,
			Object... args) {
		if (enforceMandatory)
