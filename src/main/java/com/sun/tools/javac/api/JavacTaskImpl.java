...
public class JavacTaskImpl extends JavacTask {
	...
	JavacTaskImpl(JavacTool tool, Main compilerMain, String[] args,
			Context context, List<JavaFileObject> fileObjects) {
		this.tool = tool;
	...
	JavacTaskImpl(JavacTool tool, Main compilerMain, Iterable<String> flags,
			Context context, Iterable<String> classes,
			Iterable<? extends JavaFileObject> fileObjects) {
		this(tool, compilerMain, toArray(flags, classes), context,
				toList(fileObjects));
	}
	...
	private void prepareCompiler() throws IOException {
		if (!used.getAndSet(true)) {
			...
			List<File> filenames = compilerMain.processArgs(CommandLine
					.parse(args));
			...
			com.sun.tools.javac.parser.DocCommentScanner.Factory
					.preRegister(context);
			...
			if (classes == null) {
				...
				Filter f = new Filter() {
					public void process(Env<AttrContext> env) {
						handleFlowResults(
								compiler.flow(compiler.attribute(env)), results);
					}
				};
	...
	abstract class Filter {
		void run(Queue<Env<AttrContext>> list,
			...
			ListBuffer<Env<AttrContext>> defer = ListBuffer
					.<Env<AttrContext>> lb();
