...
public class JavacTrees extends Trees {
	...
	public JCTree getTree(Element e, AnnotationMirror a, AnnotationValue v) {
		Pair<JCTree, JCCompilationUnit> treeTopLevel = elements
	...
	public boolean isAccessible(Scope scope, Element member, DeclaredType type) {
		if (scope instanceof JavacScope && member instanceof Symbol
			...
			Env<AttrContext> env = ((JavacScope) scope).env;
			return resolve.isAccessible(env,
					(com.sun.tools.javac.code.Type) type, (Symbol) member);
		} else
			return false;
	}
	...
	private Env<AttrContext> getAttrContext(TreePath path) {
		...
		// all the classes have been entered; if we're being invoked from
		// JSR269,
		...
		for (; l.nonEmpty(); l = l.tail) {
			...
			case COMPILATION_UNIT:
				...
				// ((JCCompilationUnit)tree).sourcefile);
			...
			case BLOCK: {
				...
				JCTree body = copier.copy((JCTree) tree,
						(JCTree) path.getLeaf());
