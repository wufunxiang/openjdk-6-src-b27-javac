...
public class Scanner implements Lexer {
	...
	public static class Factory {
		...
		public static final Context.Key<Scanner.Factory> scannerFactoryKey = new Context.Key<Scanner.Factory>();
	...
	/**
	 * Create a scanner from the input array. This method might modify the
	 * array. To avoid copying the input array, ensure that
	 ...
	 * @param inputLength
	 *            the size of the input. Must be positive and less than or equal
	 *            to input.length.
	 */
	protected Scanner(Factory fac, char[] input, int inputLength) {
		...
		if (inputLength == input.length) {
			if (input.length > 0
					&& Character.isWhitespace(input[input.length - 1])) {
				inputLength--;
			} else {
				...
				if (ch < '\u0080') {
					...
					if (high != 0) {
						...
						isJavaIdentifierPart = Character
								.isJavaIdentifierPart(Character.toCodePoint(
										high, ch));
					} else {
						isJavaIdentifierPart = Character
	...
	/**
	 * Scan surrogate pairs. If 'ch' is a high surrogate and the next character
	 * is a low surrogate, then put the low surrogate in 'ch', and return the
	 * high surrogate. otherwise, just return 0.
	 */
	 ...
	 * * (which is treated as the beginning of the first line). Stops positioned
	 * at the closing '/'.
	 */
	...
	protected void processComment(CommentStyle style) {
		if (scannerDebug)
			System.out.println("processComment(" + pos + "," + endPos + ","
					+ style + ")=|" + new String(getRawCharacters(pos, endPos))
					+ "|");
	}
