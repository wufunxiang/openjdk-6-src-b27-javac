...
public class JavaCompiler implements ClassReader.SourceCompleter {
	...
	private static String version(String key) {
		if (versionRB == null) {
			try {
				...
				return Log.getLocalizedString("version.resource.missing",
		...
		try {
			...
			return Log.getLocalizedString("version.unknown",
					System.getProperty("java.version"));
		}
	}
	...
	public JavaCompiler(final Context context) {
		...
		shouldStopPolicy = (options.get("shouldStopPolicy") != null) ? CompileState
		...
		if (enter.getEnv(c) == null) {
			...
			if (isPkgInfo) {
				if (enter.getEnv(tree.packge) == null) {
					String msg = log.getLocalizedString(
	 ...
	 */
	public JavaCompiler processAnnotations(List<JCCompilationUnit> roots,
			List<String> classnames) throws IOException { // TODO: see TEMP note
															// in
	...
	protected void desugar(final Env<AttrContext> env,
		 ...
		 * including information in supertypes, we need to ensure that
		 * supertypes are processed through attribute and flow before subtypes
		 * are translated.
		 */
	...
	public void reportDeferredDiagnostics() {
		if (annotationProcessingOccurred && implicitSourceFilesRead
	...
	protected void printCount(String kind, int count) {
		if (count != 0) {
			String text;
			if (count == 1)
				text = Log.getLocalizedString("count." + kind,
						String.valueOf(count));
			else
				text = Log.getLocalizedString("count." + kind + ".plural",
						String.valueOf(count));
