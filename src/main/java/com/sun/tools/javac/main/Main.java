...
public class Main {
	...
	private Option[] recognizedOptions = RecognizedOptions
			.getJavaCompilerOptions(new OptionHelper() {
				...
				public void printVersion() {
					Log.printLines(
							out,
							getLocalizedString("version", ownName,
				...
				if (targetString != null) {
					if (sourceString == null) {
						warning("warn.target.default.source.conflict",
								targetString, source.requiredTarget().name);
					} else {
	...
	public int compile(String[] args, Context context,
		...
		try {
			...
			boolean batchMode = (options.get("nonBatchMode") == null && System
	...
	void bugMessage(Throwable ex) {
		Log.printLines(out,
				getLocalizedString("msg.bug", JavaCompiler.version()));
