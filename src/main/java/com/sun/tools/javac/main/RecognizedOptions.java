...
public class RecognizedOptions {
	...
	public static class GrumpyHelper implements OptionHelper {
		...
		public void error(String key, Object... args) {
			throw new IllegalArgumentException(Main.getLocalizedString(key,
					args));
		}
	...
	static Set<OptionName> javacOptions = EnumSet.of(G, G_NONE, G_CUSTOM,
			XLINT, XLINT_CUSTOM, NOWARN, VERBOSE, DEPRECATION, CLASSPATH, CP,
			SOURCEPATH, BOOTCLASSPATH, XBOOTCLASSPATH_PREPEND,
			XBOOTCLASSPATH_APPEND, XBOOTCLASSPATH, EXTDIRS, DJAVA_EXT_DIRS,
			ENDORSEDDIRS, DJAVA_ENDORSED_DIRS, PROC_CUSTOM, PROCESSOR,
			PROCESSORPATH, D, S, IMPLICIT, ENCODING, SOURCE, TARGET, VERSION,
			FULLVERSION, HELP, A, X, J, MOREINFO,
			WERROR,
			// COMPLEXINFERENCE,
			PROMPT, DOE, PRINTSOURCE, WARNUNCHECKED, XMAXERRS, XMAXWARNS,
			XSTDOUT, XPRINT, XPRINTROUNDS, XPRINTPROCESSORINFO, XPREFER, O,
			XJCOV, XD, SOURCEFILE);

	static Set<OptionName> javacFileManagerOptions = EnumSet.of(CLASSPATH, CP,
			SOURCEPATH, BOOTCLASSPATH, XBOOTCLASSPATH_PREPEND,
			XBOOTCLASSPATH_APPEND, XBOOTCLASSPATH, EXTDIRS, DJAVA_EXT_DIRS,
			ENDORSEDDIRS, DJAVA_ENDORSED_DIRS, PROCESSORPATH, D, S, ENCODING,
			SOURCE);

	static Set<OptionName> javacToolOptions = EnumSet.of(G, G_NONE, G_CUSTOM,
			XLINT, XLINT_CUSTOM, NOWARN, VERBOSE, DEPRECATION, PROC_CUSTOM,
			PROCESSOR, IMPLICIT, SOURCE, TARGET,
			// VERSION,
			// FULLVERSION,
			// HELP,
			A,
			...
			PROMPT, DOE, PRINTSOURCE, WARNUNCHECKED, XMAXERRS, XMAXWARNS,
			// XSTDOUT,
			XPRINT, XPRINTROUNDS, XPRINTPROCESSORINFO, XPREFER, O, XJCOV, XD);
	...
	public static Option[] getAll(final OptionHelper helper) {
		return new Option[] {
				...
				// -deprecation is retained for command-line backward
				// compatibility
				...
				new XOption(DJAVA_ENDORSED_DIRS, "opt.arg.dirs",
						"opt.endorseddirs") {
					public boolean process(Options options, String option,
							String arg) {
				...
				new Option(IMPLICIT, "opt.implicit") {
					public boolean matches(String s) {
						return s.equals("-implicit:none")
								|| s.equals("-implicit:class");
					}

					public boolean process(Options options, String option,
							String operand) {
						int sep = option.indexOf(":");
						options.put(option.substring(0, sep),
								option.substring(sep + 1));
						options.put(option, option);
						return false;
					}
				},
				...
				// This option exists only for the purpose of documenting
				// itself.
				...
				// use complex inference from context in the position of a
				...
				new XOption(XSTDOUT, "opt.arg.file", "opt.Xstdout") {
					public boolean process(Options options, String option,
							String arg) {
						try {
							helper.setOut(new PrintWriter(new FileWriter(arg),
									true));
				...
				new XOption(XPREFER, "opt.prefer") {
					public boolean matches(String s) {
						return s.equals("-Xprefer:source")
								|| s.equals("-Xprefer:newer");
					}
				...
				/*
				 * -Xjcov produces tables to support the code coverage tool
				 * jcov.
				 */
				new HiddenOption(XJCOV),

				/*
				 * This is a back door to the compiler's option table. -XDx=y
				 * sets the option x to the value y. -XDx sets the option x to
				 * the value x.
				 */
				new HiddenOption(XD) {
					String s;
				 ...
				 * -XclassAsDecls is used; code elsewhere does the lookup to see
				 * if the class name is both legal and found.
				 * 
				 * In apt, the process method adds the candiate class file name
				 * to a separate list.
				 */
