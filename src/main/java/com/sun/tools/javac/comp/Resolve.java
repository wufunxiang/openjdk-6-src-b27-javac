...
public class Resolve {
	...
	public boolean isAccessible(Env<AttrContext> env, Type site, Symbol sym) {
		...
		case PROTECTED:
			...
			env.info.selectSuper && (sym.flags() & STATIC) == 0
					...
					(sym.kind != MTH || sym.isConstructor() || sym.isStatic() || ((MethodSymbol) sym)
	...
	Type rawInstantiate(Env<AttrContext> env, Type site, Symbol m,
			...
			boolean useVarargs, Warner warn) throws Infer.NoInstanceException {
		...
		if (instNeeded)
			return infer.instantiateMethod(tvars, (MethodType) mt, argtypes,
	...
	Symbol mostSpecific(Symbol m1, Symbol m2, Env<AttrContext> env, Type site,
		...
		case MTH:
			...
			boolean m1SignatureMoreSpecific = (instantiate(env, site, m2,
					types.lowerBoundArgtypes(mt1), null, allowBoxing, false,
					noteWarner) != null || useVarargs
					&& instantiate(env, site, m2,
							types.lowerBoundArgtypes(mt1), null, allowBoxing,
							true, noteWarner) != null)
					&& !noteWarner.unchecked;
			Type mt2 = types.memberType(site, m2);
			noteWarner.unchecked = false;
			boolean m2SignatureMoreSpecific = (instantiate(env, site, m1,
					types.lowerBoundArgtypes(mt2), null, allowBoxing, false,
					noteWarner) != null || useVarargs
					&& instantiate(env, site, m1,
							types.lowerBoundArgtypes(mt2), null, allowBoxing,
							true, noteWarner) != null)
					&& !noteWarner.unchecked;
	...
	Symbol findMethod(Env<AttrContext> env, Type site, Name name,
		...
		return findMethod(env, site, name, argtypes, typeargtypes,
				site.tsym.type, true, methodNotFound, allowBoxing, useVarargs,
				operator);
	}
			...
			for (Scope.Entry e = c.members().lookup(name); e.scope != null; e = e
				...
				if (e.sym.kind == MTH && (e.sym.flags_field & SYNTHETIC) == 0) {
					bestSoFar = selectBest(env, site, argtypes, typeargtypes,
							e.sym, bestSoFar, allowBoxing, useVarargs, operator);
				}
			}
			...
			if (abstractok) {
				...
				for (List<Type> l = types.interfaces(c.type); l.nonEmpty(); l = l.tail) {
					bestSoFar = findMethod(env, site, name, argtypes,
							typeargtypes, l.head, abstractok, bestSoFar,
		...
		for (; e.scope != null; e = e.next()) {
			...
			if (sym.kind == MTH) {
				...
				bestSoFar = selectBest(env, origin, argtypes, typeargtypes,
						sym, bestSoFar, allowBoxing, useVarargs, false);
			}
		}
		if (bestSoFar.exists())
			return bestSoFar;
