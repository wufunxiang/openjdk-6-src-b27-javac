...
public class Check {
	...
	protected Check(Context context) {
		...
		deprecationHandler = new MandatoryWarningHandler(log,
				verboseDeprecated, enforceMandatoryWarnings, "deprecated");
	...
	void warnDeprecated(DiagnosticPosition pos, Symbol sym) {
		if (!lint.isSuppressed(LintCategory.DEPRECATION))
			deprecationHandler.report(pos, "has.been.deprecated", sym,
					sym.location());
	}
	 ...
	 */
	public void warnUnchecked(DiagnosticPosition pos, String msg,
			Object... args) {
	...
	Type typeError(DiagnosticPosition pos, String problem, Type found,
			Type req, Object explanation) {
	...
	/**
	 * Return name of local class. This is of the form <enclClass> $ n
	 * <classname> where enclClass is the flat name of the enclosing class,
	 * classname is the simple name of the local class
	 */
	Name localClassName(ClassSymbol c) {
		for (int i = 1;; i++) {
			Name flatname = names.fromString("" + c.owner.enclClass().flatname
					+ target.syntheticNameChar() + i + c.name);
	...
	Type checkType(DiagnosticPosition pos, Type found, Type req) {
		...
		if (found.tag <= DOUBLE && req.tag <= DOUBLE)
			return typeError(pos,
					JCDiagnostic.fragment("possible.loss.of.precision"), found,
					req);
		...
		return typeError(pos, JCDiagnostic.fragment("incompatible.types"),
				found, req);
	}
		...
		if (pt == Infer.anyPoly && complexInference) {
			...
			try {
				...
				if (ex.isAmbiguous) {
					...
					log.error(pos, "undetermined.type"
							+ (d != null ? ".1" : ""), t, d);
					return syms.errType;
				} else {
					JCDiagnostic d = ex.getDiagnostic();
	...
	Type checkCastable(DiagnosticPosition pos, Type found, Type req) {
		if (found.tag == FORALL) {
			instantiatePoly(pos, (ForAll) found, req,
					castWarner(pos, found, req));
			return req;
		} else if (types.isCastable(found, req, castWarner(pos, found, req))) {
			return req;
		} else {
			return typeError(pos, JCDiagnostic.fragment("inconvertible.types"),
					found, req);
		}
	}
	...
	boolean isTypeVar(Type t) {
		return t.tag == TYPEVAR || t.tag == ARRAY
				&& isTypeVar(types.elemtype(t));
	}

	/**
	 * Check that a type is within some bounds.
	 * 
	 * Used in TypeApply to verify that, e.g., X in V<X> is a valid type
	 * argument.
	 * 
	 * @param pos
	 *            Position to be used for error reporting.
	...
	private void checkExtends(DiagnosticPosition pos, Type a, TypeVar bs) {
		if (a.isUnbound()) {
			...
			if (!types.isCastable(bs.getUpperBound(), types.upperBound(a),
					Warner.noWarnings))
				log.error(pos, "not.within.bounds", a);
	...
	Type checkClassType(DiagnosticPosition pos, Type t) {
		if (t.tag != CLASS && t.tag != ERROR)
			return typeTagError(
					...
					(t.tag == TYPEVAR) ? JCDiagnostic.fragment(
							"type.parameter", t) : t);
		else
			return t;
	}
			...
			while (args.nonEmpty()) {
				if (args.head.tag == WILDCARD)
					return typeTagError(pos,
							log.getLocalizedString("type.req.exact"), args.head);
				args = args.tail;
			}
		}
		return t;
	...
	Type checkReifiableReferenceType(DiagnosticPosition pos, Type t) {
		if (t.tag != CLASS && t.tag != ARRAY && t.tag != ERROR) {
			return typeTagError(pos,
					JCDiagnostic.fragment("type.req.class.array"), t);
	...
	Type checkRefType(DiagnosticPosition pos, Type t) {
		...
		case ERROR:
			return t;
		default:
			return typeTagError(pos, JCDiagnostic.fragment("type.req.ref"), t);
		}
	}
	 ...
	 */
	boolean checkDisjoint(DiagnosticPosition pos, long flags, long set1,
			long set2) {
	...
	long checkFlags(DiagnosticPosition pos, long flags, Symbol sym, JCTree tree) {
		...
		case TYP:
			if (sym.isLocal()) {
				...
				if (sym.name.len == 0) { // Anonymous class
					// Anonymous classes in static methods are themselves
					// static;
		...
		if (illegal != 0) {
			if ((illegal & INTERFACE) != 0) {
				...
				log.error(pos, "mod.not.allowed.here",
			...
			if (tree.type.tag == CLASS) {
				...
				while (args.nonEmpty() && forms.nonEmpty()) {
					...
					tvars_buf.append(types.substBound(((TypeVar) forms.head),
							formals, Type.removeBounds(actuals)));
				...
				while (args.nonEmpty() && tvars.nonEmpty()) {
					checkExtends(args.head.pos(), args.head.type, tvars.head);
					args = args.tail;
					tvars = tvars.tail;
				}
	...
	static Object cannotOverride(MethodSymbol m, MethodSymbol other) {
		...
		if ((other.owner.flags() & INTERFACE) == 0)
			...
			key = "clashes.with";
		return JCDiagnostic.fragment(key, m, m.location(), other,
				other.location());
	}
	 ...
	 * has a result type extended by the result type of the method it
	 * implements. Change the proxies result type to the smaller type in this
	 * case.
	 * 
	 * @param tree
	...
	void checkOverride(JCTree tree, MethodSymbol m, MethodSymbol other,
		...
		if ((m.flags() & STATIC) != 0 && (other.flags() & STATIC) == 0) {
			log.error(TreeInfo.diagnosticPositionFor(m, tree),
					"override.static", cannotOverride(m, other));
			return;
		}
		...
		boolean resultTypesOK = types.returnTypeSubstitutable(mt, ot, otres,
				overrideWarner);
		if (!resultTypesOK) {
			if (!source.allowCovariantReturns() && m.owner != origin
				...
				typeError(TreeInfo.diagnosticPositionFor(m, tree),
						JCDiagnostic.fragment("override.incompatible.ret",
								cannotOverride(m, other)), mtres, otres);
				return;
			}
		} else if (overrideWarner.warned) {
			warnUnchecked(TreeInfo.diagnosticPositionFor(m, tree),
					"prob.found.req", JCDiagnostic.fragment(
							"override.unchecked.ret",
							uncheckedOverrides(m, other)), mtres, otres);
		}
		...
		if ((other.flags() & BRIDGE) != 0) {
			log.warning(TreeInfo.diagnosticPositionFor(m, tree),
					"override.bridge", uncheckedOverrides(m, other));
		}
	...
	private boolean isDeprecatedOverrideIgnorable(MethodSymbol m,
			ClassSymbol origin) {
		// If the method, m, is defined in an interface, then ignore the issue
		// if the method
		// is only inherited via a supertype and also implemented in the
		// supertype,
		// because in that case, we will rediscover the issue when examining the
		// method
		// in the supertype.
		// If the method, m, is not defined in an interface, then the only time
		// we need to
		// address the issue is when the method is the supertype
		// implemementation: any other
		// case, we will have dealt with when examining the supertype classes
		ClassSymbol mc = m.enclClass();
		Type st = types.supertype(origin.type);
		if (st.tag != CLASS)
			return true;
		MethodSymbol stimpl = m.implementation((ClassSymbol) st.tsym, types,
				false);
		...
		for (Type t1 = sup; t1.tsym.type.isParameterized(); t1 = types
			...
			for (Scope.Entry e1 = t1.tsym.members().elems; e1 != null; e1 = e1.sibling) {
				...
				if (s1.kind != MTH
						...
						|| ((MethodSymbol) s1).implementation(site.tsym, types,
								true) != s1)
					continue;
				...
				for (Type t2 = sup; t2.tag == CLASS; t2 = types.supertype(t2)) {
					for (Scope.Entry e2 = t1.tsym.members().lookup(s1.name); e2.scope != null; e2 = e2
							.next()) {
						Symbol s2 = e2.sym;
						if (s2 == s1
								|| s2.kind != MTH
								...
								|| ((MethodSymbol) s2).implementation(
										site.tsym, types, true) != s2)
							continue;
						Type st2 = types.memberType(t2, s2);
						if (types.overrideEquivalent(st1, st2))
							log.error(pos, "concrete.inheritance.conflict", s1,
									t1, s2, t2, sup);
					}
				}
			}
	...
	public boolean checkCompatibleAbstracts(DiagnosticPosition pos, Type t1,
		...
		if (sym != null) {
			log.error(pos, "types.incompatible.diff.ret", t1, t2, sym.name
	...
	private Symbol firstIncompatibility(Type t1, Type t2, Type site) {
		...
		if (t1 == t2)
			...
			closure(t2, interfaces1,
					interfaces2 = new HashMap<TypeSymbol, Type>());
			...
			for (Scope.Entry e2 = t2.tsym.members().lookup(s1.name); e2.scope != null; e2 = e2
				...
				if (types.overrideEquivalent(st1, st2)) {
					...
					boolean compat = types.isSameType(rt1, rt2)
							|| rt1.tag >= CLASS
							&& rt2.tag >= CLASS
							&& (types.covariantReturnType(rt1, rt2,
									Warner.noWarnings) || types
									.covariantReturnType(rt2, rt1,
											Warner.noWarnings));
					if (!compat)
						return s2;
				}
	...
	void checkOverride(JCTree tree, MethodSymbol m) {
		...
		for (Type t = types.supertype(origin.type); t.tag == CLASS; t = types
				.supertype(t)) {
			TypeSymbol c = t.tsym;
			...
			if (undef != null) {
				if ((c.flags() & ENUM) != 0
					...
					MethodSymbol undef1 = new MethodSymbol(undef.flags(),
							undef.name, types.memberType(c.type, undef),
							undef.owner);
	...
	private boolean checkNonCyclicInternal(DiagnosticPosition pos, Type t) {
			...
			try {
				...
				if (c.type.tag == CLASS) {
					...
					if (clazz.interfaces_field != null)
						for (List<Type> l = clazz.interfaces_field; l
								.nonEmpty(); l = l.tail)
		...
		if (complete)
			complete = ((c.flags_field & UNATTRIBUTED) == 0)
					&& c.completer == null;
		if (complete)
	...
	private void noteCyclic(DiagnosticPosition pos, ClassSymbol c) {
		...
		Type st = types.supertype(c.type);
		if (st.tag == CLASS)
			((ClassType) c.type).supertype_field = new ErrorType(
					(ClassSymbol) st.tsym);
	...
	void validateAnnotationType(JCTree restype) {
		// restype may be null if an error occurred, so don't bother validating
		// it
		if (restype != null) {
	...
	void validateAnnotationMethod(DiagnosticPosition pos, MethodSymbol m) {
		for (Type sup = syms.annotationType; sup.tag == CLASS; sup = types
			...
			for (Scope.Entry e = s.lookup(m.name); e.scope != null; e = e
					.next()) {
				if (e.sym.kind == MTH
	...
	boolean isOverrider(Symbol s) {
		...
		for (Type sup : types.closure(owner.type)) {
			...
			for (Scope.Entry e = scope.lookup(m.name); e.scope != null; e = e
					.next()) {
	...
	public void validateAnnotation(JCAnnotation a) {
		...
		for (MethodSymbol m : members)
			if (m.defaultValue == null && !m.type.isErroneous())
				log.error(a.pos(), "annotation.missing.default.value", a.type,
						m.name);
	...
	void checkDeprecatedAnnotation(DiagnosticPosition pos, Symbol s) {
		if (allowAnnotations && lint.isEnabled(Lint.LintCategory.DEP_ANN)
	...
	int checkOperator(DiagnosticPosition pos, OperatorSymbol operator, int tag,
		...
		if (operator.opcode == ByteCodes.error) {
			log.error(pos, "operator.cant.be.applied",
					treeinfo.operatorName(tag), left + "," + right);
		}
	...
	void checkEmptyIf(JCIf tree) {
		if (tree.thenpart.getTag() == JCTree.SKIP && tree.elsepart == null
				&& lint.isEnabled(Lint.LintCategory.EMPTY))
	...
	boolean checkUnique(DiagnosticPosition pos, Symbol sym, Scope s) {
		...
		for (Scope.Entry e = s.lookup(sym.name); e.scope == s; e = e.next()) {
			if (sym != e.sym
					...
					&& (sym.kind != MTH || types.overrideEquivalent(sym.type,
							e.sym.type))) {
			...
			if ((isClassDecl || sym != e.sym) && sym.kind == e.sym.kind
				...
				if (!e.sym.type.isErroneous()) {
					...
					if (!isClassDecl) {
						if (staticImport)
							log.error(pos,
									"already.defined.static.single.import",
									what);
						else
							log.error(pos, "already.defined.single.import",
									what);
					} else if (sym != e.sym)
	...
	private class ConversionWarner extends Warner {
		...
		public ConversionWarner(DiagnosticPosition pos, String key, Type found,
				Type expected) {
			super(pos);
			this.key = key;
			this.found = found;
		...
		public void warnUnchecked() {
			...
			Check.this.warnUnchecked(pos(), "prob.found.req", problem, found,
					expected);
		}
	}

	public Warner castWarner(DiagnosticPosition pos, Type found, Type expected) {
		return new ConversionWarner(pos, "unchecked.cast.to.type", found,
				expected);
	}

	public Warner convertWarner(DiagnosticPosition pos, Type found,
			Type expected) {
