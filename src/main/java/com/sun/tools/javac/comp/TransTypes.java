...
public class TransTypes extends TreeTranslator {
	 ...
	 *            The erasure of the varargs element type, or null if
	 *            translating a non-varargs invocation
	 */
	 ...
	 */
	void addBridge(DiagnosticPosition pos, MethodSymbol meth,
			MethodSymbol impl, ClassSymbol origin, boolean hypothetical,
		...
		if (!hypothetical) {
			...
			JCExpression call = make.Apply(
					...
					translateArgs(make.Idents(md.params),
							origErasure.getParameterTypes(), null)).setType(
					calltype);
			JCStatement stat = (origErasure.getReturnType().tag == VOID) ? make
					.Exec(call) : make.Return(coerce(call,
					bridgeType.getReturnType()));
	...
	/**
	 * Add bridge if given symbol is a non-private, non-static member of the
	 * given class, which is either defined in the class or non-final inherited,
	 * and one of the two following conditions holds: 1. The method's type
	 * changes in the given class, as compared to the class where the symbol was
	 * defined, (in this case we have extended a parameterized class with
	 * non-trivial parameters). 2. The method has an implementation with a
	 * different erased return type. (in this case we have used co-variant
	 * returns). If a bridge already exists in some other class, no new bridge
	 * is added. Instead, it is checked that the bridge symbol overrides the
	 * method symbol. (Spec ???). todo: what about bridges for privates???
	 * 
	 * @param pos
	 *            The source code position to be used for the definition.
	 * @param sym
	...
	void addBridgeIfNeeded(DiagnosticPosition pos, Symbol sym,
		...
		if (sym.kind == MTH && sym.name != names.init
			...
			if (bridge == null
					|| bridge == meth
					|| (impl != null && !bridge.owner.isSubClass(impl.owner,
							types))) {
				...
				if (other != null && other != meth) {
					if (impl == null
							|| !impl.overrides(other, origin, types, true)) {
				...
				if (bridge.owner == origin
					...
					log.error(pos, "name.clash.same.erasure.no.override",
							bridge, bridge.location(origin.type, types), meth,
	...
	public void visitMethodDef(JCMethodDecl tree) {
		...
		try {
			...
			tree.body = translate(tree.body, tree.sym.erasure(types)
					.getReturnType());
	...
	public void visitNewArray(JCNewArray tree) {
		...
		tree.elems = translate(tree.elems, (tree.type == null) ? null
