...
public class Gen extends JCTree.Visitor {
			...
			case JCTree.VARDEF:
				...
				if (vdef.init != null) {
					if ((sym.flags() & STATIC) == 0) {
						...
						JCStatement init = make.at(vdef.pos()).Assignment(sym,
								vdef.init);
						...
						checkStringConstant(vdef.init.pos(),
								sym.getConstValue());
					}
				}
				break;
		...
		if (clinitCode.length() != 0) {
			MethodSymbol clinit = new MethodSymbol(STATIC, names.clinit,
					new MethodType(List.<Type> nil(), syms.voidType,
							List.<Type> nil(), syms.methodClass), c);
	...
	private void addAbstractMethod(ClassSymbol c, MethodSymbol m) {
		MethodSymbol absMeth = new MethodSymbol(m.flags() | IPROXY | SYNTHETIC,
				m.name, m.type, // was c.type.memberType(m), but now only
								// !generics supported
				c);
	...
	void genMethod(JCMethodDecl tree, Env<GenContext> env, boolean fatcode) {
		...
		// System.err.println("Generating " + meth + " in " + meth.owner);
		// //DEBUG
		if (Code.width(types.erasure(env.enclMethod.sym.type)
				.getParameterTypes())
				+ (((tree.mods.flags & STATIC) == 0 || meth.isConstructor()) ? 1
						: 0) > ClassFile.MAX_PARAMETERS) {
	...
	private int initCode(JCMethodDecl tree, Env<GenContext> env, boolean fatcode) {
		...
		meth.code = code = new Code(meth, fatcode,
				lineDebugInfo ? toplevel.lineMap : null, varDebugInfo,
				stackMap, debugCode, genCrt ? new CRTable(tree,
	...
	public void visitSwitch(JCSwitch tree) {
		...
		if (cases.isEmpty()) {
			...
			for (int i = 0; i < labels.length; i++) {
				if (l.head.pat != null) {
					int val = ((Number) l.head.pat.type.constValue())
							.intValue();
					labels[i] = val;
					if (val < lo)
						lo = val;
			...
			int startpc = code.curPc(); // the position of the selector
										// operation
			code.emitop0(opcode);
			code.align(4);
			...
			void gen() {
				if (useJsrLocally) {
					if (tree.finalizer != null) {
						...
						tryEnv.info.cont = new Chain(code.emitJump(jsr),
								tryEnv.info.cont, jsrState);
					}
	...
	void genTry(JCTree body, List<JCCatch> catchers, Env<GenContext> env) {
		...
		if (hasFinalizer) {
			...
			registerCatch(body.pos(), startseg,
					env.info.gaps.next().intValue(), catchallpc, 0);
			code.emitop0(athrow);
			code.markDead();
	...
	void genCatch(JCCatch tree, Env<GenContext> env, int startpc, int endpc,
		...
		if (startpc != endpc) {
			...
			if (startpc < endpc)
				registerCatch(tree.pos(), startpc, endpc, code.curPc(),
						catchType);
	...
	public void visitConditional(JCConditional tree) {
		...
		if (!c.isFalse()) {
			...
			if (genCrt)
				code.crt.put(tree.truepart, CRT_FLOW_TARGET, startpc,
						code.curPc());
			thenExit = code.branch(goto_);
		}
		if (elseChain != null) {
			...
			code.state.forceStackTop(tree.type);
			if (genCrt)
				code.crt.put(tree.falsepart, CRT_FLOW_TARGET, startpc,
						code.curPc());
		}
		code.resolve(thenExit);
	...
	public void visitNewClass(JCNewClass tree) {
		...
		genArgs(tree.args, tree.constructor.externalType(types)
				.getParameterTypes());
		...
		if (tree.getTag() == JCTree.NOT) {
			...
			case JCTree.PREDEC:
				...
				if (od instanceof LocalItem
					...
					((LocalItem) od).incr(tree.getTag() == JCTree.PREINC ? 1
							: -1);
					result = od;
				} else {
					od.load();
			...
			case JCTree.POSTDEC:
				...
				if (od instanceof LocalItem
						&& (operator.opcode == iadd || operator.opcode == isub)) {
					Item res = od.load();
					((LocalItem) od).incr(tree.getTag() == JCTree.POSTINC ? 1
							: -1);
					result = res;
				} else {
					Item res = od.load();
	...
	private void genNullCheck(DiagnosticPosition pos) {
		callMethod(pos, syms.objectType, names.getClass, List.<Type> nil(),
				false);
		code.emitop0(pop);
	}
	...
	Symbol getStringBufferAppend(JCTree tree, Type t) {
		...
		if (method == null) {
			method = rs.resolveInternalMethod(tree.pos(), attrEnv,
					stringBufferType, names.append, List.of(t), null);
	...
	void bufferToString(DiagnosticPosition pos) {
		callMethod(pos, stringBufferType, names.toString, List.<Type> nil(),
				false);
	}
	...
	public boolean genClass(Env<AttrContext> env, JCClassDecl cdef) {
		try {
			...
			if (generateIproxies
					...
					&& !allowGenerics // no Miranda methods available with
										// generics
			)
			...
			c.pool = pool;
			pool.reset();
			Env<GenContext> localEnv = new Env<GenContext>(cdef,
					new GenContext());
