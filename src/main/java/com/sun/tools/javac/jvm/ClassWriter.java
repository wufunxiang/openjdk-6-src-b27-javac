...
public class ClassWriter extends ClassFile {
	...
	void assembleClassSig(Type type) {
		...
		if (outer.allparams().nonEmpty()) {
			...
			sigbuf.appendName(rawOuter ? c.flatname.subName(
					c.owner.enclClass().flatname.len + 1, c.flatname.len)
					: c.name);
		} else {
	...
	RetentionPolicy getRetention(TypeSymbol annotationType) {
		...
		Attribute.Compound c = annotationType
				.attribute(syms.retentionType.tsym);
		if (c != null) {
	...
	public void writeClassFile(OutputStream out, ClassSymbol c)
		...
		boolean sigReq = typarams.length() != 0
				|| supertype.getTypeArguments().length() != 0;
