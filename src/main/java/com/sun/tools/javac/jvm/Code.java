...
public class Code {
	...
	/**
	 * Emit an opcode with two one-byte operand fields; widen if either field
	...
	public void emitop2(int op, int od) {
		...
		case putstatic:
			...
			break;
		case new_:
			state.push(uninitializedObject(
					((Symbol) (pool.pool[od])).erasure(types), cp - 3));
			break;
		case sipush:
			state.push(syms.intType);
			break;
		...
		case checkcast: {
			...
			Type t = (o instanceof Symbol) ? ((Symbol) o).erasure(types)
	...
	void emitStackMapFrame(int pc, int localsSize) {
		...
		stackMapTableBuffer[stackMapBufferSize++] = StackMapTableFrame
				.getInstance(frame, lastFrame.pc, lastFrame.locals, types);
	...
	StackMapFrame getInitialFrame() {
		...
		if (!meth.isStatic()) {
			...
			if (meth.isConstructor() && thisType != syms.objectType) {
				frame.locals[count++] = UninitializedType
						.uninitializedThis(thisType);
			} else {
	...
	public static class Chain {
		...
		/**
		 * The machine state after the jump instruction. Invariant: all elements
		 * of a chain list have the same stacksize and compatible stack and
		 * register contents.
		 */
		Code.State state;
	...
	public void resolve(Chain chain, int target) {
		...
		for (; chain != null; chain = chain.next) {
			...
			if (get1(chain.pc) == goto_ && chain.pc + 3 == target
				...
				assert !alive || chain.state.stacksize == newState.stacksize
	...
	class State implements Cloneable {
		...
		/**
		 * Force the top of the stack to be treated as this supertype of its
		 * current type.
		 */
		void forceStackTop(Type t) {
			...
			case ARRAY:
				...
				assert types.isSubtype(types.erasure(old), types.erasure(t));
		...
		State join(State other) {
			...
			for (int i = 0; i < stacksize;) {
				...
				Type result = t == tother ? t
						: types.isSubtype(t, tother) ? tother : types
								.isSubtype(tother, t) ? t : error();
				int w = width(result);
				stack[i] = result;
				if (w == 2)
